...
export const insertStreamSessionSchema = createInsertSchema(
  streamSessions
...
export const streamExports = pgTable("stream_exports", {
  public_id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  streamSessionId: varchar("stream_session_id")
    .notNull()
  ...
  poster_url: varchar("poster_url"),
  videoUrl: varchar("video_url"),
  thumbnailUrl: varchar("thumbnail_url"),
  exportedAt: timestamp("exported_at").defaultNow() || null,
  createdAt: timestamp("created_at").defaultNow(),
  processed_on: timestamp("processed_on") || null,
  status: varchar("status").notNull().default("pending"), // "pending", "processing", "completed", "failed"
});

export const insertStreamExportSchema = createInsertSchema(streamExports).omit({
  public_id: true,
  createdAt: true,
});
...
export const clips = pgTable("clips", {
  ...
  exportId: varchar("export_id")
    .notNull()
    .references(() => streamExports.public_id),
  userId: varchar("user_id")
    .notNull()
    .references(() => users.id),
...
export const connectedAccounts = pgTable(
  ...
  (table) => [
    uniqueIndex("IDX_connected_accounts_user_platform").on(
      table.userId,
      table.platform
    ),
  ]
...
export const insertConnectedAccountSchema = createInsertSchema(
  connectedAccounts
).omit({ id: true, createdAt: true });
export type InsertConnectedAccount = z.infer<
  typeof insertConnectedAccountSchema
>;
